...
struct CAC : sc_module
{
    ...
    tlm_utils::simple_initiator_socket_tagged<CAC>* initiator_socket[N_TARGETS];
        ...
        target_socket("target_socket") 
    {
        ...
        for (unsigned int i = 0; i < N_TARGETS; i++) {
            ...
            initiator_socket[i] = new tlm_utils::simple_initiator_socket_tagged<CAC>(txt);
            initiator_socket[i]->register_invalidate_direct_mem_ptr(this, &CAC::invalidate_direct_mem_ptr, i);
        }
    }
    //阻塞传输方法
    ...
    virtual bool get_direct_mem_ptr(tlm::tlm_generic_payload& trans, tlm::tlm_dmi& dmi_data) {
        ...
        if (target_id < N_TARGETS) {
            return (*initiator_socket[target_id])->get_direct_mem_ptr(trans, dmi_data);
        } else {
